...
class WeiboStyleParser:
    """
    部分微博备份 / 导出工具：仅一行时间戳，正文在后续行直到下一条时间行（无发送者列）。
    例：
    2023-01-01 12:00:00
    微博正文…
    """
    ...
    def parse(self, content: str) -> list[ParsedMessage]:
        ...
        while i < len(lines):
            ...
            if text:
                out.append(
                    ParsedMessage(text=text, sender=None, ts=ts, platform=self.platform)
                )
        return out
